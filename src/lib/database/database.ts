...
export class SurveyDatabase extends Dexie {
	...
	constructor() {
		...
		this.version(6).stores({
			users:
				'&id, identifier, startedWith, resolution.width, resolution.height, userAgent, timestamp',
			...
			points:
				'++id, userId, sessionId, timestamp, x, xL, xR, xLScreenRelative, xRScreenRelative, y, yL, yR, yLScreenRelative, yRScreenRelative, validityL, validityR, parseValidity, pupilDiameterL, pupilDiameterR, fixationDuration, fixationId',
			pageLoads: '++id, userId, stage, slide, timestamp',
			fixations: '++id, userId, fixationId, aois, duration, x, y, type, timestamp'
		});
	}
}
